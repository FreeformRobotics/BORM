...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch != 0 and epoch % 10 == 0:
            print("=> loading checkpoint '{}'".format('Obj_best_80obj_joint.pth.tar'))
            checkpoint = torch.load('Obj_best_80obj_joint.pth.tar')
            object_idt.load_state_dict(checkpoint['obj_state_dict'])
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.resume, checkpoint['epoch']))
...
def save_checkpoint(state, is_best, filename='Obj'):
    torch.save(state, filename + '_latest_80obj_joint.pth.tar')
    if is_best:
        shutil.copyfile(filename + '_latest_80obj_joint.pth.tar', filename + '_best_80obj_joint.pth.tar')
