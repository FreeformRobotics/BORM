# Developed by CEN Jun, 2020.2.15
# Functionality: DOM
# Revised by Liguang Zhou, Date: 2020.7.15
...
# Get rid of objects which rarely appear in a certain scene, which is regarded as noise
...
p_o_c = np.zeros(shape=(cls_num,150))
for i in range(cls_num):
    p_o_c[i] = num_sp[i] / num_total[i]

p_c_o = np.zeros(shape=(obj_num,cls_num))
max_dis = np.zeros(shape=(obj_num))

# Calculate the scene distribution for each object using Bayes theorem
for i in range(obj_num):
    sum=0
    for j in range(cls_num):
        sum += 1/cls_num * p_o_c[j][i]
    if sum==0:
        p_c_o[i] = 0
        continue
    for j in range(cls_num):
        p_c_o[i][j] = 1/cls_num * p_o_c[j][i]/sum
    # Calculate the discriminative value based on scene distribution
    p_c_o[i].sort()
    for j in range(6,0,-1):
        temp=p_c_o[i][j]-p_c_o[i][j-1]
        if temp>max_dis[i]:
            max_dis[i]=temp
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch != 0 and epoch % 10 == 0:
            model_path = 'weights/' + 'resnet50_best_150obj_dis'+str(args.count)+str(args.threshold)+'.pth.tar'
...
def train(train_loader, object_idt, criterion, optimizer, epoch):
    ...
    for i, (input, target, path) in enumerate(train_loader):
        ...
        for j in range(len(path)):
            ...
            for k in range(150):
                if k not in discriminative:
                    obj_hot_vector[k]=0
            obj_id_batch.append(obj_hot_vector)

        print('obj_id_batch:', obj_id_batch)
        obj_vec_batch = torch.autograd.Variable(torch.FloatTensor(obj_id_batch)).cuda()
        print('obj_vec_batch:', obj_vec_batch.shape)

        output = object_idt(obj_vec_batch)
...
def save_checkpoint(state, is_best, filename='resnet50'):
    save_path = 'weights/' + filename + '_latest_150obj_dis'+str(args.count)+str(args.threshold)+'.pth.tar'
    best_path = 'weights/' + filename + '_best_150obj_dis'+str(args.count)+str(args.threshold)+'.pth.tar'
    torch.save(state, save_path)
    # copy the save model as the best model
    if is_best:
