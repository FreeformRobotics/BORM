...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch != 0 and epoch % 10 == 0:
            print("=> loading checkpoint '{}'".format('Conv_resnet__best_150obj_joint_conv'+str(args.count)+str(args.threshold)+'.pth.tar'))
            checkpoint = torch.load('Conv_resnet__best_150obj_joint_conv'+str(args.count)+str(args.threshold)+'.pth.tar')
            model.load_state_dict(checkpoint['model'])
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.resume, checkpoint['epoch']))
...
#            matrix = matrix.reshape(1,150,150)
            matrix = np.stack((matrix,matrix,matrix),axis=0)
            obj_id_batch.append(matrix)
...
def save_checkpoint(state, is_best, filename='Conv_resnet_'):
    torch.save(state, filename + '_latest_150obj_joint_conv'+str(args.count)+str(args.threshold)+'.pth.tar')
    if is_best:
        shutil.copyfile(filename + '_latest_150obj_joint_conv'+str(args.count)+str(args.threshold)+'.pth.tar', filename + '_best_150obj_joint_conv'+str(args.count)+str(args.threshold)+'.pth.tar')
