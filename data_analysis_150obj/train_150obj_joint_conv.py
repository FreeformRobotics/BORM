...
def main():
    ...
    class Object_Linear(nn.Module):
        def __init__(self):
            ...
            self.conv3 = nn.Conv2d(128, 256, 2, stride=1, padding=0)
            self.pool1 = nn.AvgPool2d((2,2),(2,2))
            self.pool2 = nn.AvgPool2d((3,3),(3,3))
            self.pool3 = nn.AvgPool2d((3,3),(3,3))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch != 0 and epoch % 30 == 0:
            print("=> loading checkpoint '{}'".format('Conv_best_150obj_joint_dis'+str(args.count)+str(args.threshold)+'.pth.tar'))
            checkpoint = torch.load('Conv_best_150obj_joint_dis'+str(args.count)+str(args.threshold)+'.pth.tar')
            object_idt.load_state_dict(checkpoint['obj_state_dict'])
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.resume, checkpoint['epoch']))
...
def save_checkpoint(state, is_best, filename='Conv'):
