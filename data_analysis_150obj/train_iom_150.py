...
parser = argparse.ArgumentParser(description='Training of 150objects model')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch != 0 and epoch % 10 == 0:
            print("=> loading checkpoint '{}'".format('Obj_best_150obj.pth.tar'))
            checkpoint = torch.load('Obj_best_150obj.pth.tar')
            object_idt.load_state_dict(checkpoint['obj_state_dict'])
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.resume, checkpoint['epoch']))
...
#        input_var = torch.autograd.Variable(input).cuda()
        ...
        # print('obj_id_batch:', len(obj_id_batch), obj_id_batch)

        obj_vec_batch = torch.autograd.Variable(torch.FloatTensor(obj_id_batch)).cuda()
        # print('t.shape:', t.shape)      # (256,150)
...
def save_checkpoint(state, is_best, filename='Obj'):
    torch.save(state, filename + '_latest_150obj.pth.tar')
    if is_best:
        shutil.copyfile(filename + '_latest_150obj.pth.tar', filename + '_best_150obj.pth.tar')
